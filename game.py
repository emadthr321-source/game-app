...
st.set_page_config(page_title="رواد القرآن - الثانوية", layout="wide")

DATA_FILE = "league_data.json"
LOGO_PATH = "logo.png"  # مسار ملف الصورة
...
# --- عرض بنر الصورة في أعلى الواجهة ---
if os.path.exists(LOGO_PATH):
    st.image(LOGO_PATH, use_container_width=True)
else:
    st.title("🏆 رواد القرآن - المرحلة الثانوية")

# --- الشريط الجانبي: تسجيل دخول المعلم ---
...
if is_teacher:
    selected_round = st.selectbox("اختر الجولة الحالية:", list(rounds_info.keys()),
                                  index=list(rounds_info.keys()).index(data.get("round", "الجولة 1")))
...
with tab1:
    ...
    if is_teacher and 'pending_elimination' in st.session_state and st.session_state.pending_elimination:
        ...
        st.write("---")
        st.warning(
            f"⚠️ جاري إقصاء الطالب **{elim_student['name']}** من أسرة ({src_group}) ولديه ({elim_student['points']} نقطة).")

        target_group = st.selectbox("اختر المجموعة الفائزة/الخصم لتوزيع نقاط الطالب عليها بالتساوي:",
                                    [g for g in group_names if g != src_group])
